...
export default function ScriptedInterview({
	authState,
	workflowState,
	...
	async function ensurePlaybackContext() {
		...
		if (!AudioContextCtor) {
			setErrorMessage("This browser does not support audio playback.");
	...
	async function enqueuePlaybackChunk(arrayBuffer) {
		...
		// A late chunk can arrive after an interrupt; drop it instead of talking over the user.
		if (playbackEndedRef.current && !playbackActiveRef.current) {
			return;
		}
		...
		source.onended = () => {
			...
			if (playbackSourcesRef.current.size === 0 && playbackEndedRef.current) {
				playbackActiveRef.current = false;
				setIsPlaying(false);
			}
		};

		const startAt = Math.max(context.currentTime, playbackCursorRef.current || 0);
		...
		try {
			const vad = await MicVAD.new({
				onSpeechStart: () => {
					...
					// Cancels any armed end-of-turn debounce so several utterances
					// separated by short pauses commit as one answer.
				...
				onSpeechEnd: (audio) => {
					...
					const socket = socketRef.current;
					if (socket && socket.readyState === WebSocket.OPEN) {
						// Audio first: speech_end arms the commit debounce, so the
						// utterance must already be buffered server-side.
